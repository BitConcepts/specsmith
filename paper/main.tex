...
\citep{qwen3coder30b,vllmtoolcalling,hfendpoints}. Endpoint creation was
rejected before provisioning because the credential lacked
\texttt{inference.endpoints.write}; no model request or endpoint compute
occurred. This censored infrastructure attempt is archived separately and is
not evidence about model behavior. The hosted provider does not disclose its
server-side parser, so successful structured-tool probes on that route are
not described as literal \texttt{qwen3\_xml} evidence.
