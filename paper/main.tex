...
Recent work sharpens the controller hypotheses. Role-aware textual code
representations improve localization while occupying substantially less space
than raw source \citep{caumartin2026retrieval}. Five-pair tool-history pruning
improves both completion and token use in one long-horizon enterprise workflow
\citep{lodha2026lesscontext}; ACON and Memex separately study optimized
compression and indexed lossless evidence \citep{kang2025acon,
wang2026memex}. SWE-Replay branches at critical trajectory states instead of
restarting every attempt \citep{ding2026swereplay}. These results motivate
controlled transfer tests; none establishes the same effect on our tasks.

SWE-Skills-Bench reports limited average gains and sometimes substantial token
overhead from generic skill injection \citep{han2026sweskills}. This supports
our decision to keep host-agent skills outside the governance kernel and to
evaluate only mechanisms specific to requirement scope, evidence, and
verification.
...
\subsection{Preregistered v9 controller experiment}

The first study was preregistered before inference and leaves the frozen v8
publication controller unchanged. It separately ablates: (i) role/symbol/
dependency repository maps with source on demand; (ii) five recent tool pairs
plus content-addressed full-fidelity evidence; (iii) deterministic selection of
gate, compiled-patch, or milestone lanes; (iv) execution-signature early
stopping; and (v) one focused critical replay. Their combination is tested only
after each component passes one admission on the synthetic T28, fresh synthetic
T29, and pinned-upstream T30 repositories.

The promotion sequence is $n=1$, $n=5$, then independent $n=10$. Raw rows
retain context pruning, evidence references, failed-token mass, lane choice,
and handoff state. A separate cascade allows a weaker route to emit a structured
milestone handoff and a stronger route to resume the same working tree; the
hidden oracle executes exactly once after the final route. The experiment will
therefore distinguish saved context, avoided failed trajectories, and genuine
correctness transfer rather than pooling them into one favorable aggregate.

The initial six Sol admissions are diagnostic rather than claim-bearing. Only
the minimal-lane cell passed all three repositories; rolling context pruning
and terminal early-stop failed the long-horizon task, while the combined cell
failed both T29 and T30. These negative rows remain in the denominator. A
second protocol was frozen after that observation but before follow-up
inference: it preserves the complete active milestone, compacts only validated
milestones, and makes handoff terminal only when a configured stronger route
can receive the same working tree. No $n=5$ promotion is allowed until the
repaired $n=1$ cell passes every public and hidden gate.

The minimal retrieval-plus-lane follow-up passed all three admissions with
108,512 aggregate tokens, 16.4\% below the same-commit lane-only cell. In the
subsequent $n=5$ screen, T28 and T29 passed five of five while T30 passed four
of five. The failed T30 row consumed 118,283 tokens and stopped after two of
three milestones; total screen spend was 682,962 tokens. Four high-severity
audit findings therefore blocked $n=10$. A third frozen protocol tests the two
observed deterministic hotspots without changing the route, cap, validators,
or hidden oracle: explicit nullable-schema syntax for T29 and merging T30's
documentation check into its compatibility milestone.
...
\section{Training and Benchmark Expansion}

If the inference-time ablations plateau, trace-derived labels provide narrow
training targets for retrieval, compression, and escalation policies before
any attempt to fine-tune an end-to-end coding model. SWE-Gym demonstrates the
value of executable task environments and learned verifiers
\citep{pan2024swegym}; SWE-smith scales repository-grounded task synthesis
\citep{yang2025swesmith}; and SWE-Dev combines synthesized tests with
trajectory scaling for open models \citep{wang2025swedev}. We therefore plan
fresh-repository replication and held-out policy evaluation, not training on
the three scored GovernanceBench fixtures.
